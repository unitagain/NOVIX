...
# 停用词集合统一由 app.utils.stopwords.get_stopwords() 提供（中/英文 + 档案员过滤词的并集）。
# 历史上这里硬编码过两份 frozenset，已于 Phase 4 合并去重，避免多处漂移。
# Stopwords now come solely from app.utils.stopwords.get_stopwords() (CJK + English +
# archivist filter words). The former hardcoded frozensets were merged here in Phase 4.
...
def tokenize(text: str, remove_stopwords: bool = True) -> List[str]:
    ...
    # 移除停用词（统一来源：app.utils.stopwords.get_stopwords，Phase 4 三处合一）
    if remove_stopwords:
